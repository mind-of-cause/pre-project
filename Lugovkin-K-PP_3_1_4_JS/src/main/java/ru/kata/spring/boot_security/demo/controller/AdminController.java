...
public class AdminController {
    ...
    public String showEditForm(@PathVariable Long id, Model model) {
        ...
        if (userFromDb == null) {
            // Если такой пользователь не найден, например редирект на страницу ошибки
            return "redirect:/user?error=userNotFound";
        }

        Set<Long> userRolesIds = userFromDb.getRoles().stream()
                .map(Role::getId)
        ...
        return "edit"; // имя шаблона Thymeleaf для редактирования пользователя
    }
    ...
    public String updateUser(@PathVariable Long id, @ModelAttribute("user") User user, @RequestParam Set<Long> rolesIds) {
